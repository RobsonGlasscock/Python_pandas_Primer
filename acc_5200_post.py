...
# We can also use indexing to pull the first 8 rows
df.loc[:7, df.columns[2]]

# Or the 5th, 6th, and 7th, rows. 
df.loc[5:7, df.columns[2]]

# Create the same series using the name of the column.
df['sic']

# Now let's take care of the rest of them. I am using a mix of column positions and column names for illustrative purposes. Note that here we are using a Python dictionary to rename the columns. That's what the {key: value} syntax is doing.  
...
dikt= {'Floyd Mayweather': 'Welterweight', 'James Toney': 'Heavyweight'}
dikt
type(dikt)
%whos
...
for i in values:
    if i != 3:
        print('i is:', i, 'i plus one is:', i+1)
    else:
        print('Excluded')
...
# .info() gives us some more useful information about our dataframe
df.info()
...
# below I am referring to the colummns we want by feeding in a list to the dataframe.
df[list].head()

# But I can get the same results manually writing out the list. 
...
# Note for this to work we should use "inplace=True" to modify the dataframe. 
...
# Let's take a closer look at the above. We can use .loc or we can just feed in the logical condition. 
df['ret'] <= -.44

# You can see above that the logic check returns a boolean series of "False" and "True" values. False are equal to 0 and True are equal to 1. 
type(df['ret'] <= -.44)
type([df['ret'] >= -.44])

# below we can use this series and see how many 1's and 0's, (i.e., True and False values we have)
(df['ret'] <= -.44).value_counts()
df[df['ret'] <= -.44]
...
df.head()
# The minimum price is now 1.50, so it appears our function worked. Let's look again at the prices for BDL using .loc or just feeding in the logical condition which will have the dataframe only return rows where the condition is True. 
